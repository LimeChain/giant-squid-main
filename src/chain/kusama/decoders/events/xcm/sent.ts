...
import { V2Instruction_BuyExecution } from '@/chain/kusama/types/v9111';
import { V3Instruction_BuyExecution } from '@/chain/kusama/types/v1000000';
import { V4Instruction_BuyExecution, V4Instruction_DepositAsset } from '@/chain/kusama/types/v1002000';
import { V5Instruction_BuyExecution, V5Instruction_DepositAsset } from '@/chain/kusama/types/v1005000';
import { UnknownVersionError } from '@/utils/errors';
import { V2Instruction_DepositAsset } from '@/chain/kusama/types/v9160';
import { V3Instruction_DepositAsset } from '@/chain/kusama/types/v9381';
...
export class XcmSentEventPalletDecoder implements IXcmSentEventPalletDecoder {
  decode(event: Event) {
    ...
    if (sent.v9010.is(event)) {
      // not supported (1 example found)
      return;
    } else if (sent.v9100.is(event)) {
      // not supported (2 examples found)
      return;
    } else if (sent.v9111.is(event)) {
      ...
      const { origin, destination, message } = sent.v1000000.decode(event);
      if (message.length <= 1) return;
      ...
      const { origin, destination, message } = sent.v1005000.decode(event);
      if (message.length <= 1) return;
