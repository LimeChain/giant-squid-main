...
import { V4Asset, V4Junction_Parachain, V4Location } from '@/chain/moonriver/types/v2901';
...
export class TransferredAssetsEventPalletDecoder implements ITransferredAssetsEventPalletDecoder {
  decode(event: Event) {
    ...
    if (transferredAssets.v2901.is(event)) {
      const { assets, dest, fee, sender } = transferredAssets.v2901.decode(event);
