...
import { V1Junction_Parachain, V1MultiAsset, V1MultiLocation } from '@/chain/litentry/types/v9101';
import { V1MultiAsset as V1MultiAssetV2040, V1MultiLocation as V1MultiLocationV2040 } from '@/chain/litentry/types/v9150';
import { V3MultiAsset, V3MultiLocation } from '@/chain/litentry/types/v9168';
...
export class TransferredMultiAssetsEventPalletDecoder implements ITransferredAssetsEventPalletDecoder {
  decode(event: Event) {
    ...
    if (transferredMultiAssets.v9101.is(event)) {
      const { assets, dest, sender } = transferredMultiAssets.v9101.decode(event);
      return {
        from: sender,
        to: getTo(dest),
    ...
    } else if (transferredMultiAssets.v9150.is(event)) {
      const { assets, dest, sender } = transferredMultiAssets.v9150.decode(event);
      return {
        from: sender,
        to: getTo(dest),
        toChain: getDestination(dest),
    ...
    } else if (transferredMultiAssets.v9168.is(event)) {
      const { assets, dest, sender } = transferredMultiAssets.v9168.decode(event);
      return {
        from: sender,
        to: getToV3(dest),
        toChain: getDestination(dest),
...
function getDestination(destination: V1MultiLocation | V1MultiLocationV2040 | V3MultiLocation) {
...
function getAssetAmounts(assets: (V1MultiAsset | V3MultiAsset | V1MultiAssetV2040)[]) {
