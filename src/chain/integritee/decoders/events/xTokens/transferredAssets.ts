...
import { V4Asset, V4Junction_Parachain, V4Location } from '@/chain/integritee/types/integriteeParachainV520';
...
export class TransferredAssetsEventPalletDecoder implements ITransferredAssetsEventPalletDecoder {
  decode(event: Event) {
    ...
    if (transferredAssets.integriteeParachainV520.is(event)) {
      const { assets, dest, fee, sender } = transferredAssets.integriteeParachainV520.decode(event);
