...
import { IReserveTransferAssetsPalletDecoder } from '@/indexer/pallets/polkadot-xcm/calls/reserveTransferAssets';
...
export class ReserveTransferAssetsCallDecoder implements IReserveTransferAssetsPalletDecoder {
  decode(call: Call) {
    ...
    if (reserveTransferAssets.v49.is(call)) {
      const data = reserveTransferAssets.v49.decode(call);
    ...
    } else if (reserveTransferAssets.v90.is(call)) {
      const data = reserveTransferAssets.v90.decode(call);
    ...
    } else if (reserveTransferAssets.v96.is(call)) {
      const data = reserveTransferAssets.v96.decode(call);
    ...
    } else if (reserveTransferAssets.v130.is(call)) {
      const data = reserveTransferAssets.v130.decode(call);
